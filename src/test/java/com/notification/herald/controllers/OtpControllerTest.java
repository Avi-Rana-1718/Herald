...
class OtpControllerTest {
  ...
  void requestOtp_validRequest_returnsServiceStatus() throws Exception {
    OtpRequestDto dto = new OtpRequestDto(null, new OtpSmsTarget("+1234567890"), "Your OTP", 300);
  ...
  void requestOtp_blankContent_returns400() throws Exception {
    OtpRequestDto dto = new OtpRequestDto(null, new OtpSmsTarget("+1234567890"), "", 300);
  ...
  void requestOtp_nullExpiresIn_returns400() throws Exception {
    ...
    mockMvc
        .perform(post("/otp/request").contentType(MediaType.APPLICATION_JSON).content(body))
        .andExpect(status().isBadRequest());
  }
