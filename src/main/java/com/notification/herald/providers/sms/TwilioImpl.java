...
public class TwilioImpl implements SMSProvider {
  ...
  public TwilioImpl(
      @Qualifier("twilioClient") RestClient smsClient, TwilioConfiguration twilioConfiguration) {
  ...
  public String sendSMS(SMSRequestDto request) {
    ...
    TwilioResponseDto response =
        smsClient.post().body(toFormData(requestDto)).retrieve().body(TwilioResponseDto.class);
    return response.sid();
  }
