...
public class MailjetImpl implements MailProvider {
  ...
  public MailjetImpl(
      @Qualifier("mailjetClient") RestClient mailClient,
      MailjetConfiguration mailjetConfiguration) {
  ...
  private MailjetRequestDto transform(MailRequestDto requestDto) {
    ...
    MailjetRequestMessages message =
        new MailjetRequestMessages(
            new MailAddress(mailjetConfiguration.email(), mailjetConfiguration.name()),
            mailAddresses,
            requestDto.subject(),
            requestDto.content());
